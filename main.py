...
from utils.datasets import get_data, data_process
...
def main():
    ...
    data, labels, has_missing = get_data(dataset_name)
    ...
    activation = 'logistic'
    batch_size = 32
    # single hidden layer with 2*p neuros, p is the number of features
    hidden_layer = (data.shape[1] * 2, )
    ...
    for train_unl_index, test_index in sss.split(data, labels):
        ...
        if has_missing:
            X_unl_train, X_test = data_process(X_unl_train, X_test)
    ...
    print('Result: mean err: {:.1f}, and std: {:.1f}'.format(
        (1-np.mean(test_accs))*100, np.std(test_accs)*100))
